...
const navItems = ['about', 'expertise', 'projects', 'impact', 'experience', 'education', 'contact']
    ...
    <div className="min-h-screen bg-[#07111f] text-slate-200">
      <header className="fixed inset-x-0 top-0 z-50 border-b border-slate-800/70 bg-[#07111f]/85 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
          ...
          <button aria-label="Toggle navigation" aria-expanded={menuOpen} aria-controls="mobile-navigation" onClick={() => setMenuOpen(!menuOpen)} className="rounded-lg border border-slate-700 px-3 py-2 text-sm md:hidden">{menuOpen ? 'Close' : 'Menu'}</button>
        </div>
      ...
      <main>
          ...
          <div className="relative mx-auto grid max-w-7xl gap-12 px-5 pb-24 pt-16 lg:grid-cols-[1.15fr_.85fr] lg:px-8 lg:pb-32 lg:pt-24">
            <div>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                ...
                <a href="./assets/Cyfred_Odarve_Master_IT_Resume.pdf" target="_blank" rel="noopener noreferrer" className="rounded-xl border border-slate-700 bg-slate-950/40 px-5 py-3 font-semibold text-white transition hover:border-sky-400/60">View Resume</a>
                <a href="https://github.com/cydarvs" target="_blank" rel="noopener noreferrer" className="rounded-xl border border-slate-700 bg-slate-950/40 px-5 py-3 font-semibold text-white transition hover:border-sky-400/60">GitHub ↗</a>
        ...
        <section id="impact" className="border-y border-slate-800/80 bg-slate-950/30 py-24">
          <div className="mx-auto max-w-7xl px-5 lg:px-8">
            <SectionEyebrow>Engineering impact</SectionEyebrow>
            <h2 className="section-title">Production depth across infrastructure, data and delivery.</h2>
            <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {[
                ['19+', 'Years building and operating IT systems'],
                ['500GB+', 'Large production database environments'],
                ['Multi-node', 'Kubernetes / k0s operations'],
                ['AWS + Hybrid', 'Cloud and on-prem infrastructure'],
                ['CI/CD', 'Automated application delivery workflows'],
                ['Backup & DR', 'Replication, XtraBackup, binlogs and S3'],
                ['Observability', 'Prometheus, Grafana, PMM, Loki, CloudWatch'],
                ['Incident Response', 'Cross-layer production troubleshooting'],
              ].map(([value, label]) => (
        ...
        <section id="contact" className="px-5 pb-24 lg:px-8">
          <div className="mx-auto max-w-7xl overflow-hidden rounded-[2rem] border border-sky-400/20 bg-gradient-to-br from-sky-400/10 via-slate-950/70 to-indigo-500/10 p-8 md:p-12">
          <div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
            <div><SectionEyebrow>Let’s connect</SectionEyebrow>
            <h2 className="mt-2 text-3xl font-black tracking-tight text-white md:text-5xl">Building reliable systems is what I do.</h2>
            <p className="mt-5 max-w-3xl leading-7 text-slate-400">Open to senior individual-contributor, lead and management opportunities across cloud infrastructure, DevOps, systems administration, database engineering, platform engineering, solutions engineering and technical operations.</p>
            <div className="mt-6 flex flex-wrap gap-x-6 gap-y-2 text-sm text-slate-400"><span>Cagayan de Oro, Philippines</span><span>Remote / International teams</span></div></div>
            <div className="flex flex-wrap gap-3"><a href="mailto:cyfredodarve@gmail.com" className="rounded-xl bg-sky-400 px-5 py-3 font-bold text-[#07111f] hover:bg-sky-300">Email me</a>
            <a href="https://github.com/cydarvs" target="_blank" rel="noopener noreferrer" className="rounded-xl border border-slate-700 px-5 py-3 font-semibold text-white hover:border-sky-400/50">GitHub ↗</a>
            <a href="https://www.linkedin.com/in/cyfredodarve/" target="_blank" rel="noreferrer" className="rounded-xl border border-slate-700 px-5 py-3 font-semibold text-white hover:border-sky-400/50"> LinkedIn </a>
            <a href="./assets/Cyfred_Odarve_Master_IT_Resume.pdf" target="_blank" rel="noopener noreferrer" className="rounded-xl border border-slate-700 px-5 py-3 font-semibold text-white hover:border-sky-400/50">Resume ↗</a></div></div></div></section>
      </main>
